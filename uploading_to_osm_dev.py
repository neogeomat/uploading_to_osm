...
check_db_update = raw_input("Have you updated the main pseudonumber database to include the newly digitized buildings as well?(y/n)")
...
def start_upload():
    ...
    for building in json_data:
        ...
        if(osm_id):
            ...
            # check if the data is already uploaded beacuse we may be reaching the api usage limit
             
            dev_cur = dev_db.cursor()
            
            dev_cur.execute("SELECT changeset from psuedonumber where osmid=?",osm_id)
